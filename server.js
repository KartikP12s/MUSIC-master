...
app.post('/change-email', (req, res) => {
  ...
  UserModel.findOne({email: oldEmail}).then(user => {
      UserModel.findOne({email: newEmail})
      .then(existingUser => {
          if(existingUser) {
              res.send({Status: "Email already exists"})
          } else {
              UserModel.findOneAndUpdate({email: oldEmail}, {email: newEmail})
              .then(() => res.send({Status: "Success"}))
              .catch(err => res.send({Status: err}))
          }
      })
...
app.post('/check-current-password', (req, res) => {
  const {email, password} = req.body
  UserModel.findOne({email: email}).then(user => {
...
app.post('/create-playlist/:id', (req, res) => {
  const {id} = req.params
  const {playlistName, playlistDescription, user, songs} = req.body
...
app.post('/add-song/:id', (req, res) => {
  ...
  Playlist.findByIdAndUpdate({_id: id}, {$push: {songs: song}})
  .then(() => res.send({Status: "Success"}))
  .catch(err => res.send({Status: err}))
});

app.post('/update-profile-pic', upload.single('image'), (req,res)=>{
  var obj = {
  ...
  UserModel.findByEmailAndUpdate({email: req.body.email}, obj)
  .then(() => res.send({Status: "Success"}))
  .catch(err => res.send({Status: err}))
})
...
app.get('/fetch-playlists', (req, res) => {
  const {user} = req.body
  Playlist.findMany({user: user})
  .then(playlists => res.send(playlists))
  .catch(err => res.send({Status: err}))
});
app.get('/fetch-playlist/:name', (req, res) => {
  const {name, user} = req.params
  Playlist.findOne({name: name, user: user})
