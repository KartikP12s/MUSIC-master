...
const url = "mongodb+srv://devpatel2604:CubRzMISMJ0kagBE@mongotest.zudzzgr.mongodb.net/?retryWrites=true&w=majority"
const app = express();
const cors = require("cors");
app.use(cors({ origin: 'http://localhost:5173', credentials: true }));
const port = 443;
// Connect to MongoDB
mongoose.connect('mongodb+srv://devpatel2604:CubRzMISMJ0kagBE@mongotest.zudzzgr.mongodb.net/?retryWrites=true&w=majority', { useNewUrlParser: true, useUnifiedTopology: true })
...
const UserStudent = mongoose.model('UserStudent', userSchema_student, 'student');
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ username,jwtToken, role: userRole });
  } catch (error) {
...
app.get("/fetchadmins", (req, res) => {
  mon.find({ role: "admin" }, { password: 0, "__v": 0, "_id": 0 })
    .then((val) => {
      res.json(val);
    })
    .catch((err) => {
...
app.get("/fetchstaff", (req, res) => {
  monStaff.find({ role: "staff" }, { password: 0, "__v": 0, "_id": 0 })
    .then((val) => {
      res.json(val);
      console.log(val)
    })
...
app.get("/fetchstudent", (req, res) => {
  monStudent.find({ role: "student" }, { password: 0, "__v": 0, "_id": 0 })
    .then((val) => {
      res.json(val);
    })
    .catch((err) => {
...
app.delete('/deleteadmin/:username', (req, res) => {
  ...
  mon.deleteOne({ username: userName })
    .then(() => {
      res.send("Deleted");
    })
    .catch((err) => {
...
app.delete('/deletestudent/:username', (req, res) => {
  const userName = req.params.username;
  monStudent.deleteOne({ username: userName })
    .then(() => {
      res.send("Deleted");
    })
    .catch((err) => {
...
app.delete('/deletestaff/:username', (req, res) => {
  const userName = req.params.username;
  monStaff.deleteOne({ username: userName })
    .then(() => {
      res.send("Deleted");
    })
    .catch((err) => {
...
app.post('/updateAdmin/:username', (req, res) => {
  ...
  if (usernameToUpdate !== newUsername) {
    mon.findOne({ username: newUsername })
      .then((existingAdmin) => {
        if (existingAdmin) {
...
app.post('/updateStaff/:username', (req, res) => {
  ...
  monStaff.updateOne({ username: userName }, { name: name, email: email })
    .then(() => {
      res.send("Updated");
    })
    .catch((err) => {
...
app.post('/updateStudent/:username', (req, res) => {
  ...
  monStudent.updateOne({ username: userName }, { name: name, email: email })
    .then(() => {
      res.send("Updated");
    })
    .catch((err) => {
